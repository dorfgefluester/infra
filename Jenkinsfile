            ...
            steps {
                catchError(buildResult: 'SUCCESS', stageResult: 'UNSTABLE') {
                    script {
                        ...
                        if (runHappyPath) {
                            sh "${dockerRunBase} 'npx playwright test tests/e2e/smoke/user-click-paths.spec.js --project=smoke-chromium --grep \"returning player can complete key HUD interactions via click path\"'"
