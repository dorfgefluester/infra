          ...
          sh """
            ...
            ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no ${DEPLOY_USER}@${DEPLOY_HOST} '
              ...
              if [ "\$HELM_MODE" = "direct" ]; then
                ...
                HELM_STATUS=\$(sudo -n sh -lc "\"\$HELM_BIN\" --kubeconfig /etc/rancher/k3s/k3s.yaml status ${APP_NAME} -n ${K8S_NAMESPACE} 2>/dev/null | grep -m1 STATUS | awk '{print \$2}'" || true)
              fi
              if [ "\$HELM_STATUS" = "failed" ] || [ "\$HELM_STATUS" = "pending-install" ] || [ "\$HELM_STATUS" = "pending-upgrade" ]; then
                echo "INFO: removing failed/pending Helm release to allow clean install."
                if [ "\$HELM_MODE" = "direct" ]; then
                  "\$HELM_BIN" --kubeconfig /etc/rancher/k3s/k3s.yaml uninstall ${APP_NAME} -n ${K8S_NAMESPACE} --no-hooks 2>/dev/null || true
                else
